...
<div class="main-content fl-right">
    @if($search_product->count() > 0)
        <div class="alert alert-dark mt-1">Có <strong>{{$search_product->count()}}</strong> kết quả tìm kiếm với từ khóa " <strong>{{$keywords}}</strong>" </div>
        <div class="section" id="list-product-wp">
        <div class="section-detail" style="margin-top:10px">
            <ul class="list-item clearfix">
                @foreach ($search_product as $products)
                <li>
                    <a href="{{url('chi-tiet-san-pham',$products->slug)}}" title="" class="thumb">
                        <img src="{{url($products->image)}}" style="height:190px" class="bob-on-hover">
                    </a>
                    <a href="{{url('chi-tiet-san-pham',$products->slug)}}" title="" class="product-name">{{$products->name}}</a>
                    <div class="price">
                        <span class="new">{{number_format($products->price,0,'','.')}}đ</span>
                    </div>
                    <div class="action clearfix">
                        <a href="{{url('them-san-pham/'.$products->id)}}" title="Thêm giỏ hàng" class="add-cart fl-left">Thêm giỏ hàng</a>
                        <a href="{{url('chi-tiet-san-pham',$products->slug)}}" title="Mua ngay" class="buy-now fl-right">Xem ngay</a>
                    </div>
                </li>
                @endforeach
            </ul>
